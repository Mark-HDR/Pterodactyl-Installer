...
panel_conf(){
    ...
    curl -o /etc/systemd/system/pteroq.service https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pteroq.service
    ...
    if [ "$dist" = "centos" ] && { [ "$version" = "7" ] || [ "$SSLSTATUS" = "true" ]; }; then
        ...
        curl -o /etc/nginx/conf.d/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-nginx-ssl.conf
    ...
    if [ "$dist" = "centos" ] && { [ "$version" = "7" ] || [ "$SSLSTATUS" = "false" ]; }; then
        curl -o /etc/nginx/conf.d/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-nginx.conf
        sed -i -e "s@<domain>@${FQDN}@g" /etc/nginx/conf.d/pterodactyl.conf
        sed -i -e "s@/run/php/php8.3-fpm.sock@/var/run/php-fpm/pterodactyl.sock@g" /etc/nginx/conf.d/pterodactyl.conf
    ...
    if [ "$SSLSTATUS" = "true" ] && [ "$WEBSERVER" = "NGINX" ]; then
        ...
        curl -o /etc/nginx/sites-enabled/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-nginx-ssl.conf
    ...
    if [ "$SSLSTATUS" = "true" ] && [ "$WEBSERVER" = "Apache" ]; then
        ...
        curl -o /etc/apache2/sites-enabled/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-apache-ssl.conf
    ...
    if [ "$SSLSTATUS" = "false" ] && [ "$WEBSERVER" = "NGINX" ]; then
        rm -rf /etc/nginx/sites-enabled/default
        curl -o /etc/nginx/sites-enabled/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-nginx.conf
        sed -i -e "s@<domain>@${FQDN}@g" /etc/nginx/sites-enabled/pterodactyl.conf
        systemctl restart nginx
        finish
        fi
    if [ "$SSLSTATUS" = "false" ] && [ "$WEBSERVER" = "Apache" ]; then
        a2dissite 000-default.conf && systemctl reload apache2
        curl -o /etc/apache2/sites-enabled/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-apache.conf
        sed -i -e "s@<domain>@${FQDN}@g" /etc/apache2/sites-enabled/pterodactyl.conf
         a2enmod rewrite
        systemctl stop apache2
        systemctl start apache2
...
panel_install(){
    ...
    if [ "$dist" = "centos" ] && [ "$version" = "7" ]; then
        ...
        curl -o /etc/yum.repos.d/mariadb.repo https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/mariadb.repo
        ...
        curl -o /etc/php-fpm.d/www-pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/www-pterodactyl.conf
...
wings_full(){
    if [ "$dist" = "debian" ] || [ "$dist" = "ubuntu" ]; then
        ...
        curl -o /etc/systemd/system/wings.service https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/wings.service
...
phpmyadminweb(){
    ...
    if  [ "$PHPMYADMIN_SSLSTATUS" =  "true" ]; then
        rm -rf /etc/nginx/sites-enabled/default
        curl -o /etc/nginx/sites-enabled/phpmyadmin.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/phpmyadmin-ssl.conf
    ...
    if  [ "$PHPMYADMIN_SSLSTATUS" =  "false" ]; then
        curl -o /etc/nginx/sites-enabled/phpmyadmin.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/phpmyadmin.conf || exit || echo "An error occurred. cURL is not installed." || exit
...
switch(){
    if  [ "$SSLSWITCH" =  "true" ]; then
        ...
        curl -o /etc/nginx/sites-enabled/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-nginx-ssl.conf || exit || warning "Pterodactyl Panel not installed!"
    ...
    if  [ "$SSLSWITCH" =  "false" ]; then
        ...
        curl -o /etc/nginx/sites-enabled/pterodactyl.conf https://raw.githubusercontent.com/Mark-HDR/Pterodactyl-Installer/main/configs/pterodactyl-nginx.conf || exit || warning "Pterodactyl Panel not installed!"
        sed -i -e "s@<domain>@${DOMAINSWITCH}@g" /etc/nginx/sites-enabled/pterodactyl.conf
        systemctl restart nginx
        echo ""
        echo "[!] Change domains"
...
echo "https://github.com/Mark-HDR/Pterodactyl-Installer"
echo ""
